...
只做这些，其他一律不碰。Spine 阶段灰风是单进程服务，皮套 Agent 直接调 LLM，没有主控也没有蜂巢。
...
| **B 屏幕感知** | screen_sense.py + Vision API，问"我在看什么"能答 | Spine | 2天 |
...
4. persona/voice_pipeline.py：音频→VAD→ASR→组装prompt→LLM→TTS→音频输出，支持流式+打断
