...
**CR 修复闭环门禁**：CR 修复推送后，必须依次完成以下三步才算闭环，缺一不可：
1. 回复 PR review comment（DEV-68）
2. 执行出错自动落盘流程（上方）
3. 输出"CR 闭环完成"标记

未完成三步就总结收工 = 流程违规，触发 DEV-4 计数。
...
- **CR 三步闭环**（DEV-68）：CR 处理必须"修复 + 推送 + 回复 review comment"三步完成，推了代码不回复 = 未完成
