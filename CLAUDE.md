...
| Bug 修复 | 有报错/复现，≤3 文件 | 读错题本 → 实施 → CR（独立 agent）→ P0/P1 归零 |
| 多文件功能 | 跨文件但不涉及新 Module | 读错题本 → 实施 → 自验证 → CR（独立 agent）→ P0/P1 归零 → push |
...
```
--- 完成门禁 ---
自验证：[已运行验证（描述验证内容）/ 不适用（纯重构无可运行验证）]
...
3. [ ] P0/P1 必须修复；P2 逐条标注"修复"或"不修+理由"，呈现给用户确认
4. [ ] 修复后再跑独立 CR（审查范围 = 全部改动文件的完整复审，不是只看修复 diff），循环直到 CR 输出 P0=0 P1=0
5. [ ] 归零声明（每条：修复了什么 → 怎么引入的 → 归因路径 A/B/C/D → checklist 是否需要更新，四步缺一不可）
6. [ ] 门禁结论"全部通过"后才能 commit → push → 开 PR
