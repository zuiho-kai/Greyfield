...
8. **支持愿景开发模式**：开发不只围绕“当前 ticket”，还要围绕长期愿景。任何新模块都要回答：它是在强化灰风的长期人格连续性，还是只是在堆一个局部功能 demo。

### 2.1 愿景开发模式

所谓“愿景开发模式”，不是先画一张大而全蓝图，然后强行一次实现。

它的意思是：

- 愿景先行，但承诺延后
- 先把长期方向讲清楚，再把当前只做的 Spine 钉死
- 当前实现允许极简，但命名、接口、边界要给未来愿景留路
- 如果一个新能力会削弱 thread / session / handoff / original intent 这条中轴，就算它短期看起来很炫，也不应优先引入

GreyWind 当前采用的是：

`愿景开发模式 + Minimal Spine + Module 生长`

也就是：

- `architecture-v2.md` 和 `context-runtime.md` 负责守住长期愿景
- `spine-now.md` 负责守住当前边界
- 本文负责把愿景和当前实现接到一起
...
本文档是可执行的开发指令。更完整的远景设计（Agent 注册表权限体系、记忆分层细节、网页端完整页面规格、典型任务流程示例等）见原始架构文档 `archive/ai-assistant-architecture.md`，在对应 Module 实施时回查。
