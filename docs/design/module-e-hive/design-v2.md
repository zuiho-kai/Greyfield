# Module E 虫巢系统（Hive）— 设计文档 v2.1

> 创建时间：2026-03-21
> 状态：已评审，v2.2 瘦身版
> 版本：v2.2（顾问评审后精简：压缩层级、收紧赛马、分层基因库、频道折叠）
...
**灰风虫巢 = 一个具备神经进化能力的任务型多 Agent 操作系统，对外呈现 Discord 式多频道结构，对内以泰伦虫族等级制运行，低阶 Agent 通过战功积累可晋升至高阶。**
...
| 等级 | 英文名 | 中文名 | 职责 | 状态 |
|------|--------|--------|------|------|
| L3 | Overmind | 主脑 | 全局战略决策、进化仲裁、赛马触发 | 系统唯一，永不沉睡 |
| L2 | Submind | 小主脑 | 领域级战术调度、任务分解、Brood 协调 | 常驻/试验/休眠三态 |
| L1 | Brood | 虫群工作组 | 一组协同 Unit 的集合，内部直接协作 | 动态组建 |
| L0 | Unit | 战斗单位 | 专业执行者 + 原子任务执行（ToolAction） | 按领域细分多种类型 |
...
| `src/greywind/hive/evolution_engine.py` | 进化引擎：四级惩罚、战功统计 |
...
12. **Overmind 是线程级唯一主动权威** —— Vision Arbiter 不是第二主权
