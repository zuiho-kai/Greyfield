...
### DEV-68 CR 处理只推代码不回复 review `🟢`

❌ 修复了 review 指出的问题并推送，但没在 PR review comment 下回复，用户看不到处理结果
✅ CR 处理是"修复 + 推送 + 回复 review comment"三步闭环，缺一不可
> 归因：把"代码推上去"当成任务完成，忽略了 review 线程的沟通闭环
...
### DEV-70 流式清洗逻辑放在拆分之后导致跨片段匹配失败 `🟢`

❌ 在句子拆分后的 `_speak` 里用正则清洗 `<think>` block，跨行标签被拆到不同片段后正则匹配不到，推理内容泄漏到 TTS
✅ 流式文本的清洗/过滤必须在拆分之前执行。跨 chunk 的标签用状态机逐 chunk 过滤，不依赖单片段内正则匹配
> 归因 C：新场景，无对应检查项。PR #18 CR 发现
